...
현재 게시는 **Reserved VM** 입니다. 포트는 **3000 → 80** 하나만 씁니다. Autoscale용 **1104** 매핑·프록시는 Reserved VM에서 `EADDRINUSE 0.0.0.0:1104`를 일으킵니다. `.replit`에 1104와 3000을 함께 두지 않습니다.
...
- ports: `localPort = 3000` → `externalPort = 80` (이 한 줄만)
- `[env]` `PORT=3000`. 1104를 넣지 않습니다.
...
3. Replit **Publish / Deployment** 설정에서
   ...
   - 포트 매핑: 3000 → 80 만. 1104 항목은 삭제

GitHub `main`을 다시 받으면 `.replit` 값이 대시보드 설정을 덮습니다. 3000 단일 매핑이 `.replit`에 있어야 1104 충돌이 되살아나지 않습니다.
